...
def vcf_parse_with_info(variant, filters):
    """
        Parses a vcf with filters from INFO fields

        Arguments : 
            vcf : a cyvcf2 Variant from the clinvar vcf to parse
    ...
    for info_field, field_filter in filters.items():
        ...
        if info_field == "GENEINFO" and variant.INFO.get(info_field) is not None:
            ...
            filtering_ouput_dict["GENEINFO"] = any(gene.split(":")[0] in field_filter for gene in variant_genes) \
                                            or any(gene.split(":")[1] in field_filter for gene in variant_genes)
                
        #Parsing CLNREVSTAT field
        if info_field == "CLNREVSTAT" and variant.INFO.get(info_field) is not None:
            filtering_ouput_dict["CLNREVSTAT"] = variant.INFO.get(info_field) in field_filter
        ...
        if info_field == "CLNSIG" and variant.INFO.get(info_field) is not None:
            ...
            filtering_ouput_dict["CLNSIG"] = any(clnsig_value in field_filter for clnsig_value in clnsig_list)

        #Parsing CLNSIGO
        if info_field == "CLNSIGCONF" and variant.INFO.get(info_field) is not None:
            clnsigconf_list = variant.INFO.get(info_field).split(",")
            filtering_ouput_dict["CLNSIGCONF"] = any(clnsigconf_value.split("(")[0] in field_filter for clnsigconf_value in clnsigconf_list)
        
        #Parsing CLNSIGO
        if info_field == "CLNSIGINCL" and variant.INFO.get(info_field) is not None:
            clnsigconf_list = variant.INFO.get(info_field).split("|")
            filtering_ouput_dict["CLNSIGINCL"] = any(clnsigconf_value.split(":")[0] in field_filter for clnsigconf_value in clnsigconf_list) \
                                            or any()
